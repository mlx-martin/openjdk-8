...
public class HashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
    ...
    static final int DEFAULT_INITIAL_CAPACITY = 1 << 4;                 // 默认初始容量 16，必须为 2 的幂。
    static final int MAXIMUM_CAPACITY = 1 << 30;                        // 最大容量 2 的 30 次幂，必须为 2 的幂。
    static final float DEFAULT_LOAD_FACTOR = 0.75f;                     // 默认负载系数。
    static final int TREEIFY_THRESHOLD = 8;
    static final int UNTREEIFY_THRESHOLD = 6;
    static final int MIN_TREEIFY_CAPACITY = 64;                         // 至少 4 * TREEIFY_THRESHOLD 避免冲突。
     ...
     * 返回一个小于等于输入值的 2 的次方数。
     *
     * 2 的次方数的特征是 0000 0000 八位二进制数中只有 1 位是 1。
     */
    ...
    public HashMap(int initialCapacity, float loadFactor) {
        ...
        this.loadFactor = loadFactor;                       // 赋值给负载因子。
        this.threshold = tableSizeFor(initialCapacity);     // 返回一个小于等于输入值的 2 的次方数，赋值给"阈值"。
    }
